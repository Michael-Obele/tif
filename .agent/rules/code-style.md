...
- **Database**: **svelte-idb-backed compatibility adapter** for all local storage. Use the singleton exported from `src/lib/db/db.ts`; the reactive database definition lives in `src/lib/db/db.svelte.ts`.
